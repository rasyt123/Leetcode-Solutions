...
public:
    bool isPalindrome(string s) {
        ...
        */
        ...
        int l = 0;
        int r = s.size() - 1;
        bool skipalpha = !(s[l] >= 97 and s[l] <= 122);
        bool skipalpha2 =  !(s[r] >= 97 and s[r] <= 122);
        while (l < r) {
            s[l] = tolower(s[l]);
            s[r] = tolower(s[r]);
            while ((isspace(s[l]) or skipalpha) and l < s.size() - 1) {
                ++l;
            }
            while ((isspace(s[r]) or skipalpha2) and r > 0) {
                --r;
            }


            if(s[l] == s[r] or skipalpha or skipalpha2) {
                ++l;
                --r;
            } else {
                return false;
