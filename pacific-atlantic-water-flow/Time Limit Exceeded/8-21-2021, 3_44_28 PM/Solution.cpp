...
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        */
        ...
        vector<vector<bool>> dfstrue(num_rows, vector<bool>(num_cols, false));
        ...
        for (int row = 0; row <= num_rows - 1; row++) {
            for (int col = 0; col <= num_cols - 1; col++) {
                ...
                dfs(row, col, bothpaths, visited, neighbors, heights, dfstrue);
                if (bothpaths[0] and bothpaths[1]){
                    coords = {row, col};
                    result.push_back(coords);
                    coords.clear();
                    dfstrue[row][col] = true;
                } else {
                    dfstrue[row][col] = false;
                } 
                bothpaths[0] = false;
                bothpaths[1] = false;
            }
    ...
    void dfs(int row, int col, vector<bool>& bothpaths, vector<vector<bool>>& visited, vector<pair<int, int>> neighbors, vector<vector<int>>& heights, vector<vector<bool>>& dfstrue) {
        ...
        for (pair<int,int> items : neighbors) {
            if (isValid(items.first, items.second, heights) and heights[items.first][items.second] <= heights[row][col] and !visited[items.first][items.second]) {
                if (dfstrue[items.first][items.second]) {
                    bothpaths[0] = true;
                    bothpaths[1] = true;
                    return;
                } else  {
                    dfs(items.first, items.second, bothpaths, visited, neighbors, heights, dfstrue);
                }  
            }
        }
